...
### `0x52012111` 主锚点 / `dst-scripts/prefabs/world_network.lua` / 搜索信号
...
### `0x52012121` 主锚点 / `dst-scripts/prefabs/forest_network.lua` / 搜索信号

搜索 `AddComponent("weather")`。
这说明地表降水逻辑属于 `forest_network`，洞穴降水不要从本页的 `weather.lua` 推断。

### `0x52012131` 主锚点 / `dst-scripts/components/worldstate.lua` / 搜索信号
